// This sketch is a slight modifaction of https://github.com/Hansastro/Focuser
//my chames marked as //MAX in code below
// INITIALISES STEP AS EIGHTHS SO THAT SETTING fULL AND HALF STEPS WORKS
// Changed temp sensor to A0 
// added temp correct factor to allow temprature range to be corrected, 10k pot seems unneccessary
...
long tempcorrect= 178;  // MAX  change this figure to calibrate temprature
const int temperatureSensorPin = A0;  // MAX: original pin was 3 which clashed with Stepmodel pin changed to A0 as Nano uses
...
void processCommand()
{
  ...
  switch (SerialProtocol.getCommand().commandID)
  {
    ...
    case ML_GT:
      ...
      SerialProtocol.setAnswer(4, (long)(((TemperatureSensor.getTemperature()-tempcorrect) * 2)));  // mytemp correct included
      break;
    case ML_GV:
    ...
    case ML_SF:
      ...
      Motor.setStepMode(SC_EIGHTH_STEP);        // NOW INITIALISED AS THIS
      ...
      break;
    case ML_SH:
      // Set the stepping mode to half step SC_HALF_STEP
...
void setup()
{
  ...
  Motor.setStepMode(SC_EIGHTH_STEP);  // MAX Initialises STEP MODE to eighths 
  timestamp = millis();
...
void loop()
{
  ...
  {
    Display.firstPage();
    temp = TemperatureSensor.getTemperature()-tempcorrect; // my temp correct included
